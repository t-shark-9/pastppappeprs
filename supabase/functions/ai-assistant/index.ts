...
serve(async (req) => {
  ...
  try {
    ...
    if (command === "define") {
      ...
      systemPrompt = `You are a LaTeX expert. Convert natural language math expressions to proper LaTeX syntax.

RULES:
1. Return ONLY the LaTeX code. No explanations, no markdown block markers (no \`\`\`), no "Here's the latex".
2. Use smart conversions:
   - "fraction of a over b" -> \\frac{a}{b}
   - "pi" -> \\pi, "theta" -> \\theta, etc.
   - "sum of..." -> \\sum...
   - "integral of..." -> \\int...
3. Text handling:
   - If plain text is mixed with math, wrap text in \\text{}, e.g., "x is 5" -> x \\text{ is } 5
4. Multiple lines:
   - If the input implies multiple equations or steps, separate them with \\\\
   - Do NOT wrap in \\begin{align}, just provide the raw lines separated by \\\\`;
