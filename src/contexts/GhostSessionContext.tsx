...
export function GhostSessionProvider({ children }: { children: ReactNode }) {
  ...
  const [ghostUserType, setGhostUserTypeState] = useState<GhostUserType>(() => {
    const stored = localStorage.getItem(GHOST_USER_TYPE_KEY);
    return (stored as GhostUserType) || 'student';
  });
