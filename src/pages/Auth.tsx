...
import { GraduationCap, Loader2, Users, Briefcase, BookOpen, ChevronRight, Chrome } from "lucide-react";
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}${import.meta.env.BASE_URL}work`,
        },
      });
      if (error) {
        toast.error(error.message);
