...
export default function GradeYourWork() {
  ...
  const handleContinueEditing = async () => {
    ...
    try {
      ...
      const assignment = createGhostAssignment({
        title: `${SUBJECTS.find(s => s.value === subject)?.label || 'My'} ${TASK_TYPES.find(t => t.value === taskType)?.label || 'Work'}`,
        subject: subject,
        task_type: taskType,
        schoolProgram: user?.user_metadata?.school_program || 'ibdp',
        draft: { content } // Pass the content directly as part of the draft object
      });

      // Navigate to the draft page using the returned assignment ID
      navigate(`/work/assignment/${assignment.id}/draft`);
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mt-2">
                          {evaluation.overallScore >= 8 ? "Excellent work!" :
                           evaluation.overallScore >= 5 ? "Good progress, some improvements needed" :
                      ...
                      {evaluation.criteriaGrades.map((grade, i) => (
                        <div key={i} className="border rounded-lg p-4 space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`font-bold ${getScoreColor((grade.earnedMarks / grade.maxMarks) * 10)}`}>
