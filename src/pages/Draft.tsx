...
import { BlockNoteEditor, OutlineEditor, CollaboraEditor, OnlyOfficeEditor, EditorType, EDITOR_INFO } from "@/components/editors";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Save, Loader2, Sparkles, Lightbulb, FileDown, FileUp, ChevronLeft, ChevronRight, MessageSquare, ClipboardCheck, Trash2, Share2, CloudOff, FileText, List, Settings } from "lucide-react";
...
export default function Draft() {
  ...
  const [editorType, setEditorType] = useState<EditorType>(() => {
    const saved = localStorage.getItem('draft-editor-type');
    // Validate that saved value is a valid editor type
    const validTypes: EditorType[] = ['blocknote', 'outline', 'collabora', 'onlyoffice'];
    if (saved && validTypes.includes(saved as EditorType)) {
      return saved as EditorType;
    }
    return 'blocknote';
  });
  ...
  useEffect(() => {
    localStorage.setItem('draft-editor-type', editorType);
  }, [editorType]);
        ...
        <div className={`container max-w-full mx-auto ${isMobile ? 'px-3 py-2' : 'px-6 py-3'} flex items-center justify-between`}>
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <BackButton
              ...
              size="sm"
              label={!isMobile ? (isGhostAssignment ? "Dashboard" : "Back") : undefined}
            />
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" className="gap-2">
                      <span className="text-base">{EDITOR_INFO[editorType].icon}</span>
                      <span className="hidden lg:inline">{EDITOR_INFO[editorType].name}</span>
                  ...
                  <DropdownMenuContent align="end" className="w-72">
                    ...
                    {(Object.keys(EDITOR_INFO) as EditorType[]).map((type) => {
                      ...
                      const isActive = type === editorType;
                      return (
                        <DropdownMenuItem
                          key={type}
                ...
                <div className="container mx-auto px-4 py-8">
                  <div className="max-w-5xl mx-auto">
                    <div className="bg-card rounded-xl shadow-soft border border-border overflow-hidden">
                      ...
                      {editorType === 'outline' && (
                        <OutlineEditor
                          onChange={(nodes) => setContent(JSON.stringify(nodes))}
